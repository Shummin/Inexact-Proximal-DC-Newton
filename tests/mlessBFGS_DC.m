function [x_new, iter, time, objval] = mlessBFGS_DC(A, b,lambda, DC_h2, opts)
%  solves the following optimization by the proposed method with melssBFGS + semismooth Newton:
...
while iter < maxiter
    ...
    d0 =  - grad2 + (y - 2*yy/sy*s)'* grad2 * s/sy + sg/sy*y; % (d0 = -H(\nabla g(x_k)-\xi_k), H:BFGS, gamma = sy/yy)
    quad_sub = @(z)  quad_BFGS_syyy(z, x_old ,grad2, s, y, sy, yy, ss);
    U1 = sqrt(1/yy)*y ; U2 = sqrt(1/ss)*s ;
    x  = ProxB_semi( x_old, lambda, quad_sub, theta, d0, U1, U2);
    d = x - x_old ;
...
fprintf('Memoryless BFGS (S-Newton): iter = %d, time = %5.2f, objval = %8.7f, nnz(x) = %d\n', iter, time, objval, nnz(x_new));
